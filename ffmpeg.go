...
func Hls(url string, ffprobe FFprobe, gcsEvent GCSEvent) ([]string, string, error) {
	...
	message, err := ffmpeg("-y", "-v", "error", "-i", url, "-hls_segment_type", "mpegts", "-f", "hls", "-codec:", "copy", "-start_number", "0", "-hls_time", "2", "-hls_list_size", "0", "-hls_enc", "1", "-hls_enc_key", gcsEvent.Meta.Key, "-hls_enc_key_url", dir+"/"+name+".key", "-hls_enc_iv", strings.Replace(uuid.New().String(), "-", "", 10), "-master_pl_name", name+".m3u8", dir+"/"+name+"_"+".m3u8")
	return walk(dir), message, err
}
